    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-4 space-y-4 text-gray-800 dark:text-gray-200 max-w-4xl mx-auto">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-bold">Guruh qo‘shish</h1>
            </div>

            <p class="text-gray-600 dark:text-gray-400">Yangi o'quv guruhini yarating va unga o'qituvchi biriktiring.</p>
           ...
           placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none"
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="isSubmitting" class="bg-white text-black px-5 py-2 rounded-lg hover:bg-gray-100 dark:bg-white dark:text-black dark:hover:bg-gray-200 transition-colors duration-200">
                        <span v-if="isSubmitting">Yaratilmoqda...</span>
                        <span v-else>Guruh yaratish</span>
                    </Button>
                </div>
            </form>
        </div>
...
// Shadcn UI and VeeValidate imports
...
// Mock toDate if reka-ui/date is not globally available
function toDate(date: CalendarDate): Date {
    return new Date(date.year, date.month - 1, date.day);
}

// Basic `cn` utility function (replace with `clsx` and `tailwind-merge` in a real project)
...
// Date formatter for displaying the date in the button
const df = new DateFormatter('en-US', {
    dateStyle: 'long',
});

// Define props for the component, including teachers
...
const onSubmit = handleSubmit(async (formValues) => {
    ...
    router.post(route('groups.store'), payload, {
        onSuccess: () => {
            console.log('Guruh muvaffaqiyatli yaratildi!');
            resetForm();
        },
        onError: (inertiaErrors) => {
            console.error('Guruh yaratishda xatolik yuz berdi:', inertiaErrors);
        },
    });
});
...
const breadcrumbs = [
    ...
    { title: 'Guruh qo‘shish', href: '/groups/create' }
];
</script>
